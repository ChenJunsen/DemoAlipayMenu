...
public class DragGridView extends GridView {
	...
	// 最后一个 item 是否可以拖拽
	private boolean lastCanDrag = true;
	...
	private Runnable mLongClickRunnable = new Runnable() {
		...
		public void run() {
			if (mStartDragItemView != null) {
				//Modify by JasonChen at 2017-10-17 14:48
				//添加了对最后一个元素不可以拖动时的操作判断，防止出现空指针及空白情况的出现
				if(mDragPosition == getChildCount()-1 && !lastCanDrag){
					return;
				}
				isDrag = true; // 设置可以拖拽
	 ...
	 * @param lastCanDrag
	 */
	public void setLastCanDrag(boolean lastCanDrag) {
		this.lastCanDrag = lastCanDrag;
	}
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (lastCanDrag || mDragPosition != childCount - 1) {
	...
	private void onSwapItem(int moveX, int moveY) {
		...
		int childCount = getChildCount();
		if (lastCanDrag || tempPosition != childCount - 1) {
